...
while True:
    # get weather
    weather_data = get_weather('Bhubaneswar', openweather_api_key, units=units)
    weather=weather_data["weather"][0]["main"]
    t=weather_data["main"]["temp"]
    rh=weather_data["main"]["humidity"]
     # Adjusting the time received from the API to local time
    local_time = time.localtime(weather_data['dt'] + weather_data['timezone'])

    # Extracting hour and minute for display
    hours = local_time[3]
    mins = local_time[4]
